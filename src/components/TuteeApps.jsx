...
export default function TuteeApps() {
  ...
  useEffect(() => {
    axios
      ...
      .then((response) => {
        const tuteeApplications = response.data.data.filter(
          (application) =>
            application.roleApplyingFor === "Tutee" ||
            application.roleApplyingFor === "tutee"
        );
        setApplications(tuteeApplications);
      })
      .catch((error) => {
        console.log(error);
      });
